...
class NetworkTrafficAnalyzer:
    def __init__(self):
        ...
        logging.info("Inicializando ArchShield (Segurança Reforçada)...")
        self.check_rkhunter()
    ...
    def is_valid_ip(self, ip):
        """Valida se o formato do IP é correto para evitar injeções de comando/SQL"""
        try:
    ...
    def get_ip_info_from_db(self, ip):
        """Busca informações do IP no cache (Usa queries parametrizadas)"""
    ...
    def save_ip_info_to_db(self, ip, info):
        """Salva informações do IP no cache (Usa queries parametrizadas)"""
        ...
        try:
            ...
            ''', (
                ip, 
                info.get('country', 'Desconhecido'),
                info.get('city', 'Desconhecido'),
                info.get('isp', 'Desconhecido'),
                info.get('org', 'Desconhecido'),
    ...
    def get_ip_info(self, ip):
        """Busca informações detalhadas sobre um IP com validação de rede"""
        if not self.is_valid_ip(ip): return None
        ...
        return {'country': 'Erro', 'city': 'API Offline', 'isp': '-', 'org': '-', 'country_code': '', 'threat': False}
    ...
    def setup_logging(self):
        log_file = self.config["DEFAULT"].get("log_file", "/var/log/archshield.log")
        logging.basicConfig(
            filename=log_file,
            level=logging.DEBUG,
        ...
        try:
            if os.path.exists(log_file):
                with open(log_file, "r") as f:
                    if "Hidden" in f.read():
                        logging.warning("Possível Rootkit detectado pelo rkhunter!")
        except:
            pass
    ...
    def start_flask(self):
        ...
        def block_ip_route():
            ...
            return jsonify({"success": False, "error": "IP Inválido"}), 400
        ...
        def unblock_ip_route():
            ...
            if ip and self.is_valid_ip(ip) and ip in self.blocked_ips:
                self.unblock_ip(ip)
                return jsonify({"success": True})
            return jsonify({"success": False, "error": "IP Inválido ou não bloqueado"}), 400
    ...
    def process_packet(self, packet):
        with self.data_lock:
            ...
            try:
                ...
                # Otimização: Treinar a cada 100 pacotes
                if len(self.packet_features) >= 10 and len(self.packet_features) % 100 == 0:
                    X = np.array(self.packet_features[-500:])
    ...
    def block_ip(self, ip, port):
        ...
        try:
            ...
            # Usando listas no subprocess evita Shell Injection
    ...
    def start_sniffing(self):
        ...
        logging.info(f"Monitorando interface {self.interface}")
